...
void Network::BackwardPropagateError(int expected) {
	...
	for (int i = 0; i < N3; ++i) {
		outputLayerErrors.push_back((outputLayerActivations[i]-expected)*SigmoidDerivative(outputLayerActivations[i]));
	}
...
double Network::NetworkTraining(std::vector<double> inputData, int excepted) {
	...
	//for (int i = 0; i < neuronsActivations.size(); ++i) {} если выходных нейронов больше 1
...
void Network::Visualize(double result,int g) {
	...
	//canvas->FillRect(Application->MainForm->ClientRect);
	//form->ShowPoints();
	double k =  8 * pow(2*abs(result-0.5),2);
	canvas->Brush->Color= result>0.5 ? clBlue : clRed;
	switch (g) {
		case 1:
			canvas->Ellipse(form->testSet[i]->GetX()-k,form->testSet[i]->GetY()-k,
					form->testSet[i]->GetX()+k,form->testSet[i]->GetY()+k);
		i++;
		if (i==form->testSet.size()) {
			i=0;
		}
		break;

		case 2:
			canvas->Ellipse(form->resultSet[i]->GetX()-k,form->resultSet[i]->GetY()-k,
				form->resultSet[i]->GetX()+k,form->resultSet[i]->GetY()+k);
			i++;
			if (i==form->resultSet.size()) {
				i=0;
			}
    default:
		break;
