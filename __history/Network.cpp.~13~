...
void Network::BackwardPropagateErrorAndUpdateWeights(double expected, double learningRate, int x0, int x1) {
	std::vector<std::vector<double>> neuronsActivations = ActivateAndForwardPropagate(0,0);
	...
	for (int i = 0; i < N2; ++i) {
		secondLayerErrors[i]*=SigmoidDerivative(secondLayerErrors[i]);
	}
	...
	for (int i = 0; i < N1; ++i) {
		firstLayerErrors[i]*=SigmoidDerivative(firstLayerErrors[i]);
	}
	...
	for (int i = 0; i < N2; ++i) {
		for (int j = 0; j < N1+1; ++j) {
			if (j<N1) {
				network[2][i][j]-=learningRate*secondLayerErrors[i]*network[1][j][i];
			}
			else {
				network[2][i][j]-=learningRate*secondLayerErrors[i];
			}
		}
	}
}
